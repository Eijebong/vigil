...
fn proceed_replica_probe_poll_http(
    ...
    let effective_http_method =
        http_method.as_ref().map(String::as_str).unwrap_or_else(|| if body_match.is_some() { "GET" } else { "HEAD" });
    ...
    let response = match effective_http_method {
        ...
        "POST" => PROBE_HTTP_CLIENT.post(&url_bang).body(reqwest::blocking::Body::from(
